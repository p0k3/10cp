class Admin::AdvicesController < AdminController
  before_action :set_advice, only: [:show, :edit, :update, :destroy, :validate, :invalidate]
  ...
  def invalidate
    if @advice.invalidate
      redirect_to admin_advices_path, flash:{success: "Le conseil a été invalidé"}
    else
      render :edit
    end
  end
  ...
  private
    ...
    def advice_params
      params.require(:advice).permit(:title, :description, :subject_id, :author_name, :author_website, :author_email, :invalidation_reason)
    end
end
